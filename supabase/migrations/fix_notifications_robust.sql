-- 1. Enable Realtime for notifications table (Crucial for the UI balloons to show)
BEGIN;
  -- Add to publication if not already there
  ALTER PUBLICATION supabase_realtime ADD TABLE public.notifications;
EXCEPTION WHEN OTHERS THEN
  -- Already added or other minor issue, ignore
END;
COMMIT;

-- 2. Update registrar_entrada_caja_chica to be more robust and use robust name matching
...
BEGIN
  ...
  IF p_material_id IS NOT NULL THEN
      ...
      DO UPDATE SET cantidad_actual = inventario_obra.cantidad_actual + p_cantidad, ultimo_ingreso = now(), updated_at = now();
  ...
  IF p_requerimiento_id IS NOT NULL THEN
    ...
    -- Robust matching for user profile (case-insensitive and trimmed)
