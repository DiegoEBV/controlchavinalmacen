-- Migration: Actualizar el estado del detalle_sc y la cabecera_sc cuando se registra una entrada.

-- 1. Modificación v3 (Masiva con OC o sin OC)
...
DECLARE
    ...
    -- Para la actualización de estados de SC
    v_sc_req numeric;
    v_sc_consumed numeric;
    v_header_sc UUID;
...
BEGIN
    ...
    LOOP
        ...
        -- ACTUALIZAR ESTADO DEL DETALLE SC
        IF v_item.sc_detail_id IS NOT NULL THEN
            SELECT cantidad INTO v_sc_req FROM detalles_sc WHERE id = v_item.sc_detail_id;
            SELECT coalesce(SUM(cantidad), 0) INTO v_sc_consumed FROM movimientos_almacen WHERE detalle_sc_id = v_item.sc_detail_id;

            UPDATE detalles_sc
            SET estado = CASE 
                WHEN v_sc_consumed >= v_sc_req THEN 'Atendido'
                ELSE 'Parcial'
            END
    ...
    -- G. Actualizar estado de las solicitudes de compra (Cabeceras) afectadas
    FOR v_header_sc IN 
        SELECT DISTINCT sc_id 
        FROM detalles_sc 
        WHERE id IN (
            SELECT (item->>'sc_detail_id')::uuid 
            FROM jsonb_array_elements(p_items) AS item 
            WHERE item->>'sc_detail_id' IS NOT NULL
        )
    LOOP
        -- Si no existe ningún detalle de la SC que NO esté 'Atendido' ni 'Anulado'
        SELECT NOT EXISTS (
            SELECT 1 FROM detalles_sc 
            WHERE sc_id = v_header_sc AND estado != 'Atendido' AND estado != 'Anulado'
        ) INTO v_all_attended;
        
        IF v_all_attended THEN
            UPDATE solicitudes_compra SET estado = 'Atendida' WHERE id = v_header_sc AND estado != 'Atendida';
        ELSE
            -- Evaluar si alguno está parcial o atendido, de ser así, pasa a parcial
            IF EXISTS (
                SELECT 1 FROM detalles_sc 
                WHERE sc_id = v_header_sc AND (estado = 'Parcial' OR estado = 'Atendido')
            ) THEN
                UPDATE solicitudes_compra SET estado = 'Parcial' WHERE id = v_header_sc AND estado != 'Parcial';
            END IF;
        END IF;
    END LOOP;

    -- H. Actualizar estado de la Orden de Compra (si existe p_oc_id)
    IF p_oc_id IS NOT NULL THEN
        -- Si no existe ninguna línea de la OC cuyo consumo para ESA OC sea menor que la cantidad pedida
        SELECT NOT EXISTS (
            SELECT 1 
            FROM detalles_oc doc
            WHERE doc.oc_id = p_oc_id
            AND doc.cantidad > (
                SELECT COALESCE(SUM(m.cantidad), 0)
                FROM movimientos_almacen m
                WHERE m.orden_compra_id = p_oc_id 
                AND m.detalle_sc_id = doc.detalle_sc_id
            )
        ) INTO v_all_attended;
        
        IF v_all_attended AND EXISTS (SELECT 1 FROM detalles_oc WHERE oc_id = p_oc_id) THEN
            UPDATE ordenes_compra SET estado = 'Recepcionada' WHERE id = p_oc_id AND estado != 'Recepcionada';
        ELSE
            -- Evaluar si existe al menos un ingreso para esta OC
            IF EXISTS (
                SELECT 1 FROM movimientos_almacen WHERE orden_compra_id = p_oc_id
            ) THEN
                UPDATE ordenes_compra SET estado = 'Parcial' WHERE id = p_oc_id AND estado != 'Parcial';
            END IF;
        END IF;
    END IF;
...
-- 2. Modificación registrar_entrada_directa_v3 (Masiva SIN OC)
...
BEGIN
    ...
    LOOP
        ...
        IF v_item.detalle_sc_id IS NOT NULL THEN
            SELECT cantidad INTO v_sc_req FROM detalles_sc WHERE id = v_item.detalle_sc_id;
            SELECT coalesce(SUM(cantidad), 0) INTO v_sc_consumed FROM movimientos_almacen WHERE detalle_sc_id = v_item.detalle_sc_id;
    ...
    FOR v_header_sc IN 
        ...
        FROM detalles_sc 
        WHERE id IN (
            SELECT (item->>'detalle_sc_id')::uuid 
            FROM jsonb_array_elements(p_items) AS item 
            WHERE item->>'detalle_sc_id' IS NOT NULL
        )
    LOOP
        -- Si no existe ningún detalle de la SC que NO esté 'Atendido' ni 'Anulado'
        SELECT NOT EXISTS (
