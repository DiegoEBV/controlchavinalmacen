...
-- Fix: RLS Policy Always True for devoluciones

-- Fix RLS Policy for devoluciones
DROP POLICY IF EXISTS "Allow authenticated full access to devoluciones" ON public.devoluciones;
...
ON public.devoluciones FOR UPDATE 
TO authenticated 
USING (auth.uid() = usuario_id)
WITH CHECK (auth.uid() = usuario_id);

CREATE POLICY "Enable delete for owner" 
ON public.devoluciones FOR DELETE 
TO authenticated 
USING (auth.uid() = usuario_id);


-- Update Functions with explicit search_path = '' and fully qualified names
...
DECLARE
    ...
    v_year TEXT;
BEGIN
    v_year := pg_catalog.TO_CHAR(pg_catalog.NOW(), 'YYYY');
    ...
    RETURN 'V-' || v_year || '-' || pg_catalog.LPAD(v_count::TEXT, 4, '0');
END;
$$;
...
BEGIN
    ...
    -- Process Deliveries
    FOR v_item_entrega IN SELECT * FROM pg_catalog.JSONB_ARRAY_ELEMENTS(p_items_entrega)
    LOOP
        -- Get Detail Record
        ...
        INSERT INTO public.movimientos_almacen (
            ...
            pg_catalog.NOW(), v_pedido.destino_o_uso, v_pedido.solicitante_nombre, v_pedido.solicitante_dni,
            v_pedido.tercero_id, v_pedido.encargado_id, v_pedido.bloque_id, v_pedido.numero_vale, pg_catalog.NOW()
        );
...
BEGIN
    ...
    RETURN 'V-' || v_year || '-' || pg_catalog.LPAD((COALESCE(v_count, 0) + 1)::TEXT, 4, '0');
END;
$$;
...
BEGIN
    ...
    v_vale_num := 'V-' || v_year || '-' || pg_catalog.LPAD(v_count::TEXT, 4, '0');
    ...
    RETURN pg_catalog.JSONB_BUILD_OBJECT('id', v_pedido_id, 'numero_vale', v_vale_num);
END;
$$;
